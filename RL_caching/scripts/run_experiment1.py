...
def main():
    ...
    parser.add_argument("--num-requests", type=int, default=100000)
    ...
    parser.add_argument("--exclude-top-k", type=int, default=None,
                         help="usado so com --skew-mode exclude_top_k. Quantos dos arquivos mais populares "
                              "do catalogo ficam de fora do servidor 0. Default = --capacity (o que um "
                              "cache guloso por hit rate escolheria).")
    ...
    with open(args.out, "w", newline="") as f, ProcessPoolExecutor(max_workers=args.workers) as ex:
        futures = {
            ex.submit(run_one_config, tt, ns, args.catalog_size, args.num_requests, capacity,
                      ...
                      args.skew_mode, args.exclude_top_k, args.workload_mode, args.workload_sigma): (tt, ns, run_id)
